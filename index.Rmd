...
if (is.null(meta_out)) {
  ...
  if (length(grep("pdf", names(meta_out)))) {
    french <- meta_out$`csasdowntex:::sr_pdf`$french
    options(french = french)
    prepub <- meta_out$`csasdowntex:::sr_pdf`$prepub
  } else if (length(grep("word", names(meta_out)))) {
    french <- meta_out$`csasdowntex::sr_word`$french
    options(french = french)
    prepub <- meta_out$`csasdowntex::sr_word`$prepub
  }
}
...
```{r output-options}
keep_non_CC <- TRUE           # Make preliminary CC report

...
input_data_table <- function(tab, cap = "", translate = FALSE){
  # Source column
...
#'
#' @return a [csasdown::csas_table()]
total_landed_catch_table <- function(tab, by_vec, cap = "", translate = FALSE){
  tab <- tab %>%
    filter(year >= 2009) %>%
...
#'
#' @return a [csasdown::csas_table()]
sok_harvest_table <- function(tab, by_vec, first_yr, cap = "", translate = FALSE){
  tab <- tab %>%

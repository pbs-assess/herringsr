...
A Bayesian framework was used to estimate time series of spawning biomass, instantaneous natural mortality, and age-2 recruitment from `r major_start_yr` to `r major_end_yr`.
Advice to managers for the major SARs includes posterior estimates of current stock status ($\mli{SB}_{`r major_end_yr`}$), stock status relative to the LRP of $0.3\mli{SB}_0$, and spawning biomass in `r major_end_yr + 1` assuming no catch ($\mli{SB}_{`r major_end_yr + 1`}$).

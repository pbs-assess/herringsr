...
In 2017, an updated version of the SCA model was applied to assess each of the `r number_to_word(length(major_regions_short))` major Pacific Herring stocks [@cleary2018].
...
Advice to managers for the major stock areas includes posterior estimates of current stock status ($\mli{SB}_{`r minor_end_yr`}$), spawning biomass in `r assess_yr + 1` assuming no catch ($\mli{SB}_{`r assess_yr + 1`}$), and stock status relative to the LRP of $0.3\mli{SB}_0$.
...
examination of the Bayes posterior shows the prior is not updated for HG, CC, SoG, and WCVI SARs and the estimated value reflects the prior mean [Appendix D; @cleary2018].
...
Application of the AM1 model would remove such safeguards despite recent simulation evaluation showing that large (positive) assessment errors are produced by the current assessment model even with $q_2 = 1$ [@dfo2018].
...
For these reasons, advice presented here is based on the AM2 stock assessment model parameterization, supported also by comparisons presented in Table A1, [@dfo2016] and Appendix D, [@cleary2018].

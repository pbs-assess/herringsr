...
Advice to managers for the major SARs includes posterior estimates of current stock status ($\mli{SB}_{`r minor_end_yr`}$), stock status relative to the LRP of $0.3\mli{SB}_0$, and spawning biomass in `r assess_yr + 1` assuming no catch ($\mli{SB}_{`r assess_yr + 1`}$). The projected spawning biomass is based on the current year's recruitment deviations from average (using a Beverton-Holt stock-recruit model) and estimated natural mortality and weight-at-age, both averaged over the most recent 5-years.

...
statistical catch-age model in the Central Coast major stock assessment region.
...
(ref:cc-priors-posts-figure-cap) Prior probability distributions (lines) with
...
(ref:cc-priors-posts-figure-cap-fr) French caption.

```{r cc-priors-posts-figure, fig.cap=ifelse(french, "(ref:cc-priors-posts-figure-cap-fr)", "(ref:cc-priors-posts-figure-cap)")}
model_ind <- match(en2fr("CC", french), major_regions_short)
make.priors.posts.plot(major_models[[model_ind]],
                       priors.only = FALSE)
```

(ref:cc-traces-figure-cap) Trace plots for Markov chain Monte Carlo output of
...
(ref:cc-traces-figure-cap-fr) French caption.

```{r cc-traces-figure, fig.asp=1, fig.cap=ifelse(french, "(ref:cc-traces-figure-cap-fr)", "(ref:cc-traces-figure-cap)")}
make.traces.plot(major_models[[model_ind]],
                 axis.lab.freq = 200)
```

(ref:cc-autocor-figure-cap) Auto-correlation plots for Markov chain Monte Carlo output of
estimated parameters for the Central Coast major stock assessment region.
See Table \@ref(tab:hg-parameter-est-table) and
Figure \@ref(fig:cc-traces-figure) for description.

(ref:cc-autocor-figure-cap-fr) French caption.

```{r cc-autocor-figure, fig.asp=1, fig.cap=ifelse(french, "(ref:cc-autocor-figure-cap-fr)", "(ref:cc-autocor-figure-cap)")}
make.autocor.plot(major_models[[model_ind]])
```

(ref:cc-pairs-figure-cap) Pairs plots for Markov chain Monte Carlo output of
estimated parameters for the Central Coast major stock assessment region.
See Table \@ref(tab:hg-parameter-est-table) and
Figure \@ref(fig:cc-traces-figure) for description.

(ref:cc-pairs-figure-cap-fr) French caption.

```{r cc-pairs-figure, fig.asp=1, fig.cap=ifelse(french, "(ref:cc-pairs-figure-cap-fr)", "(ref:cc-pairs-figure-cap)")}
make.pairs.plot(major_models[[model_ind]])
```

(ref:cc-biomass-retro-figure-cap) Maximum posterior density of spawning biomass
with retrospectives for the Central Coast major stock assessment region.

(ref:cc-biomass-retro-figure-cap-fr) French caption.

```{r cc-biomass-retro-figure, fig.cap=ifelse(french, "(ref:cc-biomass-retro-figure-cap-fr)", "(ref:cc-biomass-retro-figure-cap)")}
biomass.plot.mpd(major_models[[model_ind]],
                 xlim = c(1972, assess_yr))
```

(ref:decision-table-cc) `r paste0("Projected spawning biomass for ", assess_yr + 1, " compared to harvest rate and TAC for the Central Coast region, rows 1-4 in Table \\@ref(tab:decision-table-cc). Red horizontal and vertical lines show median harvest rate, TAC, and projected $\\SB_t$; mean statistics shown with green lines.")`
...
(ref:decision-table-cc-2) `r paste0("Projected spawning biomass for ", assess_yr + 1, " compared to harvest rate and TAC for the Central Coast region, rows 5-8 in Table \\@ref(tab:decision-table-cc). Red horizontal and vertical lines show median harvest rate, TAC, and projected $\\SB_t$; mean statistics shown with green lines.")`
...
(ref:decision-table-cc-3) `r paste0("Projected spawning biomass for ", assess_yr + 1, " compared to harvest rate and TAC for the Central Coast region, rows 9-10 in Table \\@ref(tab:decision-table-cc). Red horizontal and vertical lines show median harvest rate, TAC, and projected $\\SB_t$; mean statistics shown with green lines.")`

...
tab <- inp_catch
# Source column
tab$Source <- en2fr(tab$Source, french, allow_missing = TRUE)
tmp <- tab$Source
...
tmp[!is.na(bracs)] <- paste0(en2fr(firstup(nonbracs[!is.na(nonbracs)]), french),
                             " (",  firstlower(en2fr(firstup(bracs[!is.na(bracs)]), french)), ")")
tab$Source <- tmp
# Data column
tmp <- strsplit(tab$Data, ": *")
tmp <- lapply(tmp, function(x){
  j <- firstup(x)
  j <- en2fr(j, translate = french, allow_missing = TRUE)
  if(length(j) > 1){
    j <- c(j[1], tolower(j[-1]))
    j <- paste(j, collapse = ": ")
  }
...
tab <- major_catch %>% 
  ...
  rename(Year = year)
tab <- add_cols_and_reorder(tab, by = major_regions_short)
names(tab)[names(tab) == "Year"] <- en2fr("Year", french)
csas_table(tab,
           format = "latex",
           align = c("l", rep("r", 5)),
           caption = cap)
...
names(tab) <- en2fr(names(tab), french, allow_missing = TRUE)
csas_table(tab,
           format = "latex",

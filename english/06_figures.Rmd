...
=======
Nous n'incluons que les RES avec des prises accidentelles déclarées.
>>>>>>> Stashed changes
...
(ref:spawn-section-cc-cap) Spawn index in 
thousands of tonnes (t) for selected sections  `r major_start_yr` to `r major_end_yr`
in the Central Coast major stock assessment region scaled to abundance by $q$ (median posterior estimate).
The dashed vertical line delineates between two periods defined by the dominant survey method: surface surveys (`r major_start_yr` to `r new_surv_yr-1`), and
...
(ref:spawn-section-cc-cap-fr) Indice de frai en 
milliers de tonnes (t) pour des sections sélectionnées `r major_start_yr` à `r major_end_yr` dans la région d'évaluation du stock majeur de la côte centrale, mis à l'échelle de l'abondance par $q$ (estimation médiane postérieure).

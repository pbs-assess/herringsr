...
increase (up arrow), decrease (down), or
neither in the last five years,
where there are more than two years of data.
Status indicates if recent average values are
greater than one SD above (plus), below (minus), or
neigher the time series average.
Note that
pressure indicators are for Hecate Strait and Queen Charlotte Sound,
responses are for Haida Gwaii (HG) herring, and
sea surface temperature (SST) is from the
\link{https://www.avl.class.noaa.gov/release/data_available/avhrr/index.htm}
{AVHRR satellite}.
Legend: prey abundance and biomass are in
...
Pacific Hake biomass is from the
hake stock assessment and acoustic trawl survey
...
weight of age-3 herring [@dfo2021c]; and
...
La tendance indique une augmentation (flèche vers le haut),
une diminution (flèche vers le bas) significative, ou
ni l'un ni l'autre au cours des cinq dernières années,
lorsqu'il y a plus de deux années de données.
L'état indique si les valeurs moyennes récentes sont
supérieures à une ET au-dessus (plus), au-dessous (moins), ou
ni l'un ni l'autre à la moyenne de la série chronologique.
Il convient de noter que
les indicateurs de pression concernent
le détroit d'Hécate et le détroit de la Reine-Charlotte,
les réponses sont pour le hareng de Haida Gwaii (HG), et
la température de surface de la mer (TSM) provient du
\link{https://www.avl.class.noaa.gov/release/data_available/avhrr/index.htm}
{satellite AVHRR}.

...
(ref:major-wa-cap) Mean weight-at-age in kilograms (kg) `r major_start_yr` to `r major_end_yr` in major stock assessment regions.
Lines show `r nRollWA`-year running means for age-`r age_first` to age-`r age_plus` herring, incrementing up from bottom line and shaded from darker to lighter, except thickest line shows age-`r age_highlight` herring. Circles show mean for age-`r age_highlight` herring.
...
(ref:major-pa-cap) Proportion-at-age from `r major_start_yr` to `r major_end_yr`
...
(ref:major-spawn-index-cap) Spawn index in thousands of tonnes (t) from `r major_start_yr` to `r major_end_yr` in major stock assessment regions.
...
(ref:major-spawn-index-cap-fr) L’indice du frai en milliers de tonnes de `r major_start_yr` à `r major_end_yr` dans les régions d’évaluation des stocks majeurs.
...
scaled abundance (points, spawn index scaled by median posterior estimate of $q$).
\textbf{Panel (b)}: Posterior estimates of
instantaneous natural mortality rate (year-1).
...
(yellow = RoeSN; turquoise = RoeGN; purple = other, see Fig \@ref(fig:major-catch)).
...
\textbf{Panels (d and f)}: Red lines indicate the median limit reference point
$0.3\SB_0$, where $SB_0$ is the estimated unfished spawning biomass.
Notes: biomass and catch are in thousands of tonnes (t), and 
all bands and error bars are `r ci_level*100`% credible intervals.}
...
(ref:spawn-section-cc-cap) Scaled abundance in thousands of tonnes (t) for
...
<!-- TODO: Needs editing -->
(ref:spawn-section-cc-cap-fr) Indice de frai en milliers de tonnes (t) pour les sections sélectionnées de `r major_start_yr` à `r major_end_yr`
...
`r major_start_yr` to `r major_end_yr` in major stock assessment regions.
...
(ref:proj-biomass-density-cap) Posterior distributions of projected spawning biomass assuming no fishing in `r assess_yr+1` $\SB_{`r assess_yr+1`}$ in thousands of tonnes (t) in major stock assessment regions.
...
(ref:proj-biomass-density-cap-fr) Les distributions postérieures de la biomasse reproductrice projetée en supposant qu’il n’y aura pas de pêche en `r assess_yr+1` $\SB_{`r assess_yr+1`}$ en milliers de tonnes dans les régions d’évaluation des stocks majeurs.

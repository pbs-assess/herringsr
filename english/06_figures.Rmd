...
(ref:major-catch-cap) Total landed catch in thousands of tonnes (t) `r recent_catch_yr` to `r major_end_yr` in major stock assessment regions.
...
Legend: 'Other' represents the reduction, the food and bait, and the special use fishery;
...
in aquaculture activities in thousands of fish 
...
(ref:major-wa-cap) Mean weight-at-age for Pacific Herring in kilograms (kg) `r major_start_yr` to `r major_end_yr` in major stock assessment regions.Lines show `r nRollWA`-year running means for age-`r age_first` to age-`r age_plus` herring, incrementing up from bottom line and shaded from darker to lighter, except thickest line shows age-`r age_highlight` herring. Circles show mean for age-`r age_highlight` herring.
...
(ref:major-pa-cap) Proportion-at-age for Pacific Herring from `r major_start_yr` to `r major_end_yr`
...
(ref:major-spawn-index-cap) Spawn index in thousands of tonnes (t) for Pacific Herring from `r major_start_yr` to `r major_end_yr` in major stock assessment regions.
...
\textbf{Panel (a)}: Model fit (lines; median posterior estimate) to
scaled abundance (points).
Spawn index is scaled to abundance by the median posterior estimate of $q$.
\textbf{Panel (b)}: Posterior estimates of
instantaneous natural mortality rate ($\text{year}^{-1}$).
...
\textbf{Panel (d)}: Posterior estimate of spawning biomass. 
Point is median posterior forecast spawning biomass in `r assess_yr + 1`
in the absence of fishing.
Coloured vertical bars indicate commercial catch, excluding spawn-on-kelp
(see figure \ref{fig:major-catch}).
\textbf{Panel (e)}: Estimated log recruitment deviations from
...
points and lines are median posterior estimates,
bands and error bars are `r ci_level * 100`\% credible intervals, and
...
<!-- Haida Gwaii Storyboard -->
(ref:storyboard-hg-cap) Haida Gwaii stock assessment region time series output
...
<!-- Prince Rupert District Storyboard -->
(ref:storyboard-prd-cap) Output from the Pacific Herring statistical catch-age model `r major_start_yr` to `r major_end_yr`
...
<!-- Central Coast Storyboard -->
(ref:storyboard-cc-cap) Output from the Pacific Herring statistical catch-age model `r major_start_yr` to `r major_end_yr`
...
(ref:spawn-section-cc-cap) Scaled abundance in thousands of tonnes (t) of Pacific Herring for
selected sections from `r major_start_yr` to `r major_end_yr`
in the Central Coast major stock assessment region.
...
(ref:harvest-rate-cap) Effective harvest rate $U_t$ for 
`r major_start_yr` to `r major_end_yr` for Pacific Herring in major stock assessment regions.
...
(ref:proj-biomass-density-cap) Posterior distributions of projected spawning biomass of Pacific Herring assuming no fishing in `r assess_yr+1` $\SB_{`r assess_yr+1`}$ in thousands of tonnes (t) in major stock assessment regions.

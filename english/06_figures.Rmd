...
(ref:major-incidental-cap) Incidental catch from
`r min(ic_yrs)` to `r max(ic_yrs)`: major.

(ref:major-incidental-cap-fr) Incidental catch in French from
`r min(ic_yrs)` to `r max(ic_yrs)`: major.

```{r major-incidental, fig.asp=ifelse(french, 1, 1), fig.cap = ifelse(french, "(ref:major-incidental-cap-fr)", "(ref:major-incidental-cap)")}
...
<!-- Haida Gwaii Storyboard -->
(ref:storyboard-hg-cap) Output from the Pacific Herring statistical catch-age model
from `r major_start_yr` to `r major_end_yr`
in the Haida Gwaii major stock assessment region.
Panel (a): model fit (lines; median posterior estimate) to scaled abundance
(points; see Figure \@ref(fig:major-spawn-index)).
The spawn index is scaled to abundance by the median posterior estimate
of the spawn survey scaling parameter $q$.
Panel (b): posterior estimates of instantaneous natural mortality rate $M$ (year^-1^).
Panel (c): reconstructed age-`r age_first` recruitment in thousands of millions from
`r major_start_yr + age_first` to `r major_end_yr`.
Horizontal line and shaded area indicate median and `r ci_level*100`%
credible interval for unfished age-`r age_first` recruitment $R_0$, respectively.
Panel (d): posterior estimate of spawning biomass.
Point and vertical line indicate the median and `r ci_level*100`%
...
Panels (b & d): lines and shaded areas indicate medians and `r ci_level*100`%
credible intervals, respectively.
Panel (e): log recruitment deviations from
`r major_start_yr + age_first` to `r major_end_yr`.
Panels (c & e): points and vertical lines indicate medians and
`r ci_level*100`% credible intervals, respectively.
Panel (f): phase plot of spawning biomass production from
`r new_surv_yr` to `r assess_yr - 1` (median posterior estimates).
Grey shading becomes darker in chronological order;
...
```{r spawn-section-cc, fig.asp=ifelse(french, 1, 1), fig.cap = ifelse(french, "(ref:spawn-section-cc-cap-fr)", "(ref:spawn-section-cc-cap)")}
plot_spawn_section(

...
# ggsave(filename = here("figures", paste0("catchFullTimeSeries.png")),  width=7, height=9, dpi=300)

catch_plot_pre70 <- plot_catch(
  df = gear_4,
  xlim = c(special_start_yr, 1969), # major_end_yr),
  translate = french
)
# ggsave(filename = here("figures", paste0("catch_pre1970.png")),  width=7, height=9, dpi=300)

catch_plot_post69 <- plot_catch(
  df = gear_4,
  xlim = c(1970, major_end_yr),
  translate = french
)
# ggsave(filename = here("figures", paste0("catchShortTimeSeries.png")),  width=7, height=9, dpi=300)
```
...
```{r coastwide-biomass-catch-ppt, fig.asp=1, fig.cap="", eval = slides}
plot_coastwide_biomass_catch(
  models = major_models,
  catch = major_catch,
  reg_names = major_regions_full,
  translate = french)
ggsave(filename = here(knitr_figs_dir, "coastwide_biomass_catch.png"),  
...
blue circles and shaded regions indicate a productive period
(Table \ref{tab:prod-period});
blue lines indicate proportion of spawning biomass
during a productive period #1 $\overline{\SB}_\mli{Prod}$; and
...
\storyboard{`r ifelse(prod_period[[model_ind]]$prop == 1, "", prod_period[[model_ind]]$prop)`}
...
```{r storyboard-hg, fig.asp=ifelse(french, 1.0, 1.05), fig.cap = ifelse(french, "(ref:storyboard-hg-cap-fr)", "(ref:storyboard-hg-cap)"), eval = keep_non_CC}
cowplot::plot_grid(
  plot_a_hg,
  plot_b_hg,
  plot_c_hg,
...
```{r ppt-fnc, eval = TRUE}
create_slide_figures <- function(model_index = 1) {
  
  region_abbreviation <-regions$Region[model_index]
  
  plot_a_multi <- plot_scaled_abundance(
    major_surv %>% filter(region == major_regions_full[model_index]),
    major_models[[model_index]],
    surv_type,
    new_surv_yr = new_surv_yr,
    xlim = xlim_all,
    show_x_axis = TRUE, 
    y_axis_label_newline_length = 20,
    prod_yrs = prod_period[[model_index]]$yrs,
    translate = french)
  
  plot_a_multi_x_up <- plot_a_multi  + scale_x_continuous(position = "top")
  
  plot_b_multi <- plot_natural_mortality(
    major_models[[model_index]],
    xlim = xlim_all,
    show_x_axis = TRUE,
    y_max = M_max,
  ...
  plot_d_multi <- plot_biomass_catch(
    major_models[[model_index]],
    catch_df = ct,
    xlim = xlim_all,
    y_axis_label_newline_length = 20,
    label_points = TRUE,
    prod_yrs = prod_period[[model_index]]$yrs,
    prop_prod = prod_period[[model_index]]$prop,
    translate = french)
  
  plot_e_multi <- plot_recruitment_devs(
    major_models[[model_index]],
    run_mean_yrs = nRollRec,
    xlim = xlim_all,
    y_axis_label_newline_length = 20,
  ...
  plot_f_multi <- plot_biomass_phase(
    major_models[[model_index]],
    catch_df = ct,
    new_surv_yr = new_surv_yr,
    y_axis_label_newline_length = 20,
  ...
  ggsave(filename = here(knitr_figs_dir, 
    paste0("storyboard_", region_abbreviation, "_ppt.png")),  
    width=12, height=6, dpi=300)
  
  shortstory_ppt <- cowplot::plot_grid(
    plot_a_multi_x_up,
    plot_b_multi_x_up,
    plot_c_multi,
    plot_d_multi,
  ...
  ggsave(filename = here(knitr_figs_dir, 
    paste0("shortstory_", region_abbreviation, "_ppt.png")),  
    width=8, height=5, dpi=300)
  
  plot_a_single <- plot_scaled_abundance(
    major_surv %>% filter(region == major_regions_full[model_ind]),
    ...
    translate = french)
  ggsave(filename = here(knitr_figs_dir,
    paste0("plot_a_", region_abbreviation, "_ppt.png")), 
    plot = plot_a_single,  width=10, height=5, dpi=300)
  ...
  plot_d_single <- plot_biomass_catch(
    ...
    show_prod_usr = TRUE,
    show_prod_yrs = TRUE,
    col_catch = FALSE, #colours catch bars
    prod_yrs = prod_period[[model_index]]$yrs,
    prop_prod = prod_period[[model_index]]$prop,
    translate = french) 
  
  ggsave(filename = here(knitr_figs_dir, 
    paste0("plot_d_", region_abbreviation, "_ppt.png")), 
    plot = plot_d_single,  width=9, height=5, dpi=300)
}

```

```{r FSAR-fnc, eval = TRUE}
create_FSAR_figures <- function(model_index = 1) {
  region_abbreviation <-regions$Region[model_index]
  ...
  shortstory_FSAR <- cowplot::plot_grid(
    plot_a_multi_x_up,
    plot_b_multi_x_up,
    plot_c_multi,
    plot_d_multi,
  ...
  ggsave(filename = here(knitr_figs_dir,
                         paste0("shortstory_", region_abbreviation, "_FSAR.png")),
         width=8, height=5, dpi=300)
}

```

```{r  ppt-plots-hg, fig.cap = "(ref:storyboard-hg-cap)", eval= slides}
create_slide_figures(model_index = model_ind)
```

<!-- Prince Rupert District Storyboard -->
```{r scaled-abundance-prd, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Prince Rupert District Storyboard -->
```{r natural-mortality-prd, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Prince Rupert District Storyboard -->
```{r recruitment-prd, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Prince Rupert District Storyboard -->
```{r biomass-catch-prd, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Prince Rupert District Storyboard -->
```{r recruitment-devs-prd, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Prince Rupert District Storyboard -->
```{r biomass-phase-prd, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
```{r storyboard-prd, fig.asp=ifelse(french, 1.0, 1.05), fig.cap = ifelse(french, "(ref:storyboard-prd-cap-fr)", "(ref:storyboard-prd-cap)"), eval = keep_fsar & keep_non_CC}
cowplot::plot_grid(
  plot_a_prd,
  plot_b_prd,
  plot_c_prd,
...
```{r ppt-plots-prd, fig.cap = "(ref:storyboard-prd-cap)", eval= slides}
create_slide_figures(model_index = model_ind)
```
...
```{r ppt-plots-cc, fig.cap = "(ref:storyboard-cc-cap)", eval= slides}
create_slide_figures(model_index = model_ind)
```

<!-- Strait of Georgia Storyboard -->
```{r scaled-abundance-sog, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Strait of Georgia Storyboard -->
```{r natural-mortality-sog, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Strait of Georgia Storyboard -->
```{r recruitment-sog, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Strait of Georgia Storyboard -->
```{r biomass-catch-sog, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Strait of Georgia Storyboard -->
```{r recruitment-devs-sog, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
<!-- Strait of Georgia Storyboard -->
```{r biomass-phase-sog, fig.show = "hide", fig.asp = 1, eval = keep_fsar}
...
```{r storyboard-sog, fig.asp=ifelse(french, 1.0, 1.05), fig.cap = ifelse(french, "(ref:storyboard-sog-cap-fr)", "(ref:storyboard-sog-cap)"), eval = keep_fsar & keep_non_CC}
cowplot::plot_grid(
  plot_a_sog,
  plot_b_sog,
  plot_c_sog,
...
```{r ppt-plots-sog, fig.asp= 1.1, fig.cap = "(ref:storyboard-sog-cap)", eval= FALSE}
create_slide_figures(model_index = model_ind)
```

```{r FSAR-plots-sog, fig.asp= 1.1, eval= FALSE}
create_FSAR_figures(model_index = model_ind)
```
...
```{r storyboard-wcvi, fig.asp=ifelse(french, 1.0, 1.05), fig.cap = ifelse(french, "(ref:storyboard-wcvi-cap-fr)", "(ref:storyboard-wcvi-cap)"), eval = keep_non_CC}
cowplot::plot_grid(
  plot_a_wcvi,
  plot_b_wcvi,
  plot_c_wcvi,
...
```{r ppt-plots-wcvi, fig.asp = 1.1, fig.cap = "(ref:storyboard-wcvi-cap)", eval= slides}
create_slide_figures(model_index = model_ind)
```
...
Points and vertical lines indicate
medians and `r ci_level*100`% credible intervals for $U_t$, respectively.
Horizontal dashed lines indicate $U_t = `r target_hr`$.
<!-- , and -->
<!-- horizontal dotted lines indicate $`r prop_f_msy` F_\text{MSY}$ where -->
<!-- $F_\text{MSY}$ is fishing mortality at maximum sustainable yield. -->
...
fsar_ind <- match(en2fr(fsar_sars, french), major_regions_short)
if(keep_fsar) {
  m_c <- major_catch
  m_m <- major_models
  m_r_f <- major_regions_full
} else {
  keep_regions <- major_regions_full[-fsar_ind]
  m_c <- major_catch %>% filter(region %in% keep_regions)
  m_m <- major_models[-fsar_ind]
  m_r_f <- keep_regions
}
plot_harvest_rate(
  df = m_c,
...
sog_ind <- match(en2fr("SoG", french), major_regions_short)
if(keep_fsar) {
  m_m <- major_models
  m_r_f <- major_regions_full
} else {
...
```{r major-wa-res-doc, fig.asp=ifelse(FALSE, 1.1, 1.15), fig.cap = ifelse(FALSE, "(ref:major-wa-cap-fr)", "(ref:major-wa-cap)"), eval=FALSE}
wa_HG <- read_csv(here("../DataSummaries/Summaries/fleetWtAgeHG.csv")) 
wa_PRD <- read_csv(here("../DataSummaries/Summaries/fleetWtAgePRD.csv")) 
wa_CC <- read_csv(here("../DataSummaries/Summaries/fleetWtAgeCC.csv"))
wa_SOG <- read_csv(here("../DataSummaries/Summaries/fleetWtAgeSOG.csv")) 
wa_WCVI <- read_csv(here("../DataSummaries/Summaries/fleetWtAgeWCVI.csv"))

wa_graph <- function(df) {
  plot_wa_by_gear(
  df = df,
  xlim = c(1951, 2022),
  ylim = wa_ylim,
...
# wa_HGplot <- wa_graph(wa_HG)
# ggsave(filename = here("../SISCAH_resDoc2023/figure", paste0("weight_ageHG.png")),  width=8, height=10.5, dpi=300)
# 
# wa_SOGplot <- wa_graph(wa_SOG)
# ggsave(filename = here("../SISCAH_resDoc2023/figure", paste0("weight_ageSOG.png")),  width=8, height=10.5, dpi=300)
# 
# wa_PRDplot <- wa_graph(wa_PRD)
# ggsave(filename = here("../SISCAH_resDoc2023/figure", paste0("weight_agePRD.png")),  width=8, height=10.5, dpi=300)
# 
# wa_CCplot <- wa_graph(wa_CC)
# ggsave(filename = here("../SISCAH_resDoc2023/figure", paste0("weight_ageCC.png")),  width=8, height=10.5, dpi=300)
# 
# wa_WCVIplot <- wa_graph(wa_WCVI)
# ggsave(filename = here("../SISCAH_resDoc2023/figure", paste0("weight_ageWCVI.png")),  width=8, height=10.5, dpi=300)
```

```{r major-pa-res-doc, fig.asp=ifelse(french, 1.2, 1.25), fig.cap = ifelse(french, "(ref:major-pa-cap-fr)", "(ref:major-pa-cap)"), eval=FALSE}

pa_HG <- read_csv(here("../DataSummaries/Summaries/ageDataHG.csv")) 
pa_SOG <- read_csv(here("../DataSummaries/Summaries/ageDataSOG.csv"))
pa_PRD <- read_csv(here("../DataSummaries/Summaries/ageDataPRD.csv"))
pa_CC <- read_csv(here("../DataSummaries/Summaries/ageDataCC.csv"))
pa_WCVI <- read_csv(here("../DataSummaries/Summaries/ageDataWCVI.csv"))

pa_graph <- function(df){
  plot_pa_by_gear(
  df = df,
  age_plus = age_plus,
  conf = ci_level,
...
# pa_HGplot <- pa_graph(pa_HG)
# ggsave(filename = here("figures", paste0("proportion_of_ageHG.png")),  width=8, height=10.5, dpi=300)
# 
# pa_SOGplot <- pa_graph(pa_SOG)
# ggsave(filename = here("figures", paste0("proportion_of_ageSOG.png")),  width=8, height=10.5, dpi=300)
# 
# pa_PRDplot <- pa_graph(pa_PRD)
# ggsave(filename = here("figures", paste0("proportion_of_agePRD.png")),  width=8, height=10.5, dpi=300)
# 
# pa_CCplot <- pa_graph(pa_CC)
# ggsave(filename = here("figures", paste0("proportion_of_ageCC.png")),  width=8, height=10.5, dpi=300)
# 
# pa_WCVIplot <- pa_graph(pa_WCVI)
# ggsave(filename = here("figures", paste0("proportion_of_ageWCVI.png")),  width=8, height=10.5, dpi=300)
```

(ref:wcvi-eco-concept-one-cap) Conceptual model of the
annual life cycle of Pacific Herring,
including predation mortality in inshore and offshore regions,
spawn timing (January to June), recruitment, and fisheries.
Arrows to predator boxes (grey) indicate losses
due to predation mortality from Pacific Hake (HAKE), Humpback Whales (HBW),
Steller Sea Lions (SSL), and Harbour Seals (HS).
Spawning occurs at the end of each year on June 30^th^.
Arrows from recruitment boxes (blue) indicate recruitment of juveniles
hatched from eggs deposited during the previous year’s spawning events.
Recruits do not migrate offshore during the feeding season
but are subject to inshore predation before mixing with age-2+ herring
returning from the offshore (green arrow on far left)
during the spawning season.
The top panel shows average weekly (vertical bars) and
cumulative (dashed line) proportion of spawn
for inshore spawning grounds, along with timing for roe and SOK fisheries.

(ref:wcvi-eco-concept-one-cap-fr) Concept 1.

```{r wcvi-eco-concept-one, fig.cap = ifelse(french, "(ref:wcvi-eco-concept-one-cap-fr)", "(ref:wcvi-eco-concept-one-cap)"), out.width = "100%"}
knitr::include_graphics(
  here(ifelse(french, "figures/wcvi_eco_concept_1.png", "figures/wcvi_eco_concept_1.png"))
)
```

(ref:wcvi-eco-predators-cap) Estimated predator consumption of
WCVI Herring from 1951 to 2024.
Consumption is estimated in thousands of tonnes for each species,
with Pacific Hake and Humpback Whales broken into
two and four feeding groups, respectively.
Feeding groups for hake differ in their Herring size preference,
while Humpback Whale seasonal groups differ in size preference and
spatio-temporal overlap with WCVI Herring.

(ref:wcvi-eco-predators-cap-fr) Predators.

```{r wcvi-eco-predators, fig.cap = ifelse(french, "(ref:wcvi-eco-predators-cap-fr)", "(ref:wcvi-eco-predators-cap)"), out.width = "100%"}
knitr::include_graphics(
  here(ifelse(french, "figures/wcvi_eco_predators.png", "figures/wcvi_eco_predators.png"))
)
```

(ref:wcvi-eco-quilt-cap) Spatial distribution of Pacific Herring
spawn (blended index) from 1951 to 2025
for West Coast of Vancouver Island herring
in Statistical Areas 23, 24, and 25
broken into individual sections.
Diamonds indicate incomplete survey of spawning event, and
colour gradients show spawning magnitude.

(ref:wcvi-eco-quilt-cap-fr) Quilt.

```{r wcvi-eco-quilt, fig.cap = ifelse(french, "(ref:wcvi-eco-quilt-cap-fr)", "(ref:wcvi-eco-quilt-cap)"), out.width = "100%"}
knitr::include_graphics(
  here(ifelse(french, "figures/wcvi_eco_quilt.png", "figures/wcvi_eco_quilt.png"))
)
```

(ref:wcvi-eco-concept-two-cap) An annual time-step
in the three-area Pacific Herring predation mortality model
showing herring migrations between inshore spawning areas
in Pacific Fishery Management Areas (PFMAs) and offshore feeding grounds,
spawn timing (January to June), recruitment, fisheries, and predation.
Arrows to predator boxes (grey) indicate losses due to predation mortality
from Pacific Hake (HAKE), Humpback Whales (HBW), Steller Sea Lions (SSL),
and Harbour Seals (HS).
The annual time-step runs from July 1^st^ to June 30^th^
with spawning occurring at the end of each year.
Arrows from recruitment boxes (blue) indicate
recruitment of juveniles hatched from eggs deposited
during the previous year’s spawning events.
Recruits do not migrate offshore during the feeding season,
but are subject to inshore predation before mixing with age-2+ herring
returning from the offshore (green arrow on far left)
during the spawning season.
The top panel shows average weekly (vertical bars) and
cumulative (dashed line) proportion of spawn for
inshore spawning grounds, along with timing for roe and SOK fisheries.

(ref:wcvi-eco-concept-two-cap-fr) Concept 2.

```{r wcvi-eco-concept-two, fig.cap = ifelse(french, "(ref:wcvi-eco-concept-two-cap-fr)", "(ref:wcvi-eco-concept-two-cap)"), out.width = "100%"}
knitr::include_graphics(
  here(ifelse(french, "figures/wcvi_eco_concept_2.png", "figures/wcvi_eco_concept_2.png"))
)
```
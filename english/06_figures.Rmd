...
greater than one SD above ($+$) or below ($-$) the time series average.
Note that
pressure indicators are for Hecate Strait and Queen Charlotte Sound, and
responses are for Haida Gwaii (HG) herring.
Legend: sea surface temperature (SST; \link{https://www.avl.class.noaa.gov/release/data_available/avhrr/index.htm}{AVHRR satellite});
...
HG_StellerSeaLion_Abund is interpolated estimates of Steller Sea Lion abundance
(S. Tucker, DFO, pers. comm.);
Hake_Acoustic is Pacific Hake biomass from the hake stock assessment
and joint United States (US)-Canada acoustic trawl survey by
US agencies NOAA, NMFS, and AFSC (1995 to 2001),
NOAA, NMFS, and NWFSC (2003 to 2019), and
DFO (1995 to 2019)
(S. Johnson, Landmark Fisheries Research, pers. comm.);
IPHC is International Pacific Halibut Commission (A. Edwards, DFO, pers. comm.);
HGHerringSB_Med is median HG herring spawning biomass [@dfo2021c];
Age3_herring_weight_g is mean weight in grams of age-3 HG herring [@dfo2021c]; and
Mean_LWResids_Female is residuals from a double log-transformed
length-weight regression for female HG herring (Boldt et al.).$^{\ref{fn:boldt}}$

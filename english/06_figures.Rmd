...
(ref:major-wa-cap) Time series of Pacific Herring means for weight-at-age in kilograms (kg) for dominant age-`r age_highlight` (circles) and `r nRollWA`-year running means for age-`r age_first` to age-`r age_plus` herring (lines, incrementing up from bottom line, `r major_start_yr` - `r major_end_yr`). The age-`r age_plus` class includes fish ages `r age_plus` and older. Age classes are shaded to reflect their contribution to the stock in general. In years where there are no biological samples for an age class, values are imputed as the mean of the previous `r nRollWA` years, except for the beginning of the time series which are imputed by extending the first non-missing value backwards. Only seine-net collected samples (commercial and test) are included due to size-selectivity of gillnets. Note: vertical axes are cropped `r wa_ylim[1]` to `r wa_ylim[2]` kg.
...
(ref:major-pa-cap) Time series of Pacific Herring proportion-at-age from `r major_start_yr` to `r major_end_yr` in the major stock assessment regions. Dot size and colour indicates age class proportion for the year. Each year adds up to 100%.
The gray line is the mean age, and the shaded area is the approximate `r ci_level*100`% credible interval for the mean age.
Biological summaries only include samples collected using seine nets (commercial and test) due to size-selectivity of other gear types such as gillnet.
The age-`r age_plus` class is a 'plus group' which includes fish ages `r age_plus` and older.
...
<!-- Once the english and french changes are refined and accepted copy and paste to other regions -->

(ref:storyboard-hg-cap) Haida Gwaii stock assessment region time series output (`r major_start_yr` - `r major_end_yr`) for the Pacific Herring statistical catch-age model.\
Panel (a): Model fit (lines; median posterior estimate) Spawn survey abundance index (points, scaled by median posterior estimate of $q = ?$).\
Panel (b): Posterior estimates of instantaneous natural mortality rate (year-1).\
Panel (c): Reconstructed number of age-`r age_first` recruits in thousands of millions, from 1953 onwards. Horizontal line is median for unfished age-`r age_first` recruitment $R_0$.\
Panel (d): Posterior estimate of spawning biomass. Point is median of forecast spawning biomass in `r assess_yr+1` in the absence of fishing. Coloured vertical bars indicate commercial catch, excluding spawn-on-kelp (yellow = RoeSN; turquois = RoeGN; purple = other see Fig \@ref(fig:major-catch)).\
Panel (e): Estimated log recruitment deviations from 1953 onwards.\
Panel (f): Spawning biomass production (dive survey period `r new_surv_yr`-`r assess_yr-1`; median posterior estimates), chronologically shaded light to dark; triangle indicates `r assess_yr-1`.\
Notes: 	Biomass and catch are in thousands of tonnes (t).\
Bands and error bars are all `r ci_level*100`% credible intervals.\
Panels (d & f): Red lines indicate the median limit reference point of 0.3*(estimated unfished spawning biomass). 

...
plot_a_multi_x_up <- plot_scaled_abundance(major_surv %>% filter(region == major_regions_full[model_ind]),
                                   major_models[[model_ind]],
                                   surv_type,
                                   new_surv_yr = new_surv_yr,
                                   xlim = xlim_all,
...
plot_b_multi_x_up <- plot_natural_mortality(major_models[[model_ind]],
                                    xlim = xlim_all,
                                    y_max = M_max,
                                    y_axis_label_newline_length = 20,
                                    show_x_axis = TRUE,
...
plot_proj_biomass_density(major_models, 
                          ...
                          translate = french)
```
```{r stacked-biomass, fig.asp=ifelse(french, 1.2, 1.2), fig.cap = ifelse(french, "(ref:stacked-biomass-cap-fr)", "(ref:stacked-biomass-cap)")}

#need install.packages("library(gcookbook)") for stacked graphs.

unique(major_catch$region)

french <- FALSE
my_catch <- major_catch %>%
  filter(year <= major_end_yr) %>%
  group_by(year, region) %>%
  summarise_at(vars(value),
               list(value = sum)) 
my_catch$region <- recode_factor(my_catch$region, 
                                 "Haida Gwaii" = "HG", 
                                 "Prince Rupert District" = "PRD", 
                                 "Central Coast" = "CC", 
                                 "Strait of Georgia" = "SOG",
                                 "West Coast of Vancouver Island" = "WCVI")
unique(my_catch$year)
zero_catch <- expand.grid(year = c(my_catch$year), region = unique(my_catch$region), value = 0)

my_catch <- bind_rows(my_catch, zero_catch)%>%
  group_by(year, region) %>%
...
g <- ggplot(my_catch, aes(x = year, y = value, fill = region)) +
        geom_area()+
        ylab("Catch 1000 t") +
...
#different colours
g2 <- ggplot(my_catch, aes(x = year, y = value, fill = region)) + 
...
ggsave(filename = here("figures", paste0("catch_stacked.png")), plot = g,  width=9, height=5, dpi=300)

hg.mod <- major_models[[1]]
prd.mod <- major_models[[2]]
cc.mod <- major_models[[3]]
sog.mod <- major_models[[4]]
wcvi.mod <- major_models[[5]]
year <- hg.mod$mpd$yrs
HG <- hg.mod$mpd$sbt
PRD <- prd.mod$mpd$sbt
CC <- cc.mod$mpd$sbt
SOG <- sog.mod$mpd$sbt
WCVI <- wcvi.mod$mpd$sbt

df <- data.frame(year, HG, PRD, CC, SOG, WCVI) %>%
        head(-1)#remove next years prediction

df <- gather(df, key = "region", value = "spawning_biomass", HG:WCVI, factor_key=TRUE)
        
spawning_biomass_stacked <- ggplot(df, aes(x = year, y = spawning_biomass, fill = region)) +
  geom_area() +
  ylab(newline_format("Spawning Biomass 1000 t", 15)) +
  scale_x_continuous(breaks = seq(from = 1900, to = 2100, by = 10),
        name = NULL,
        position = "bottom") +
  theme(axis.text=element_text(size=16),
...
ggsave(filename = here("figures", paste0("spawning_biomass_stacked.png")), plot = spawning_biomass_stacked,  width=9, height=4.5, dpi=300)

z <- cowplot::plot_grid(g, spawning_biomass_stacked, ncol = 1, align = "v")

ggsave(filename = here("figures", paste0("summed_picture.png")), plot = z,  width=11, height=6, dpi=300)

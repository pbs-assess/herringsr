...
Notes: figure may include data outside SAR boundaries and
...
Notes: La figure exclut les RES pour lesquels
...
```{r ppt-plots-sog, fig.asp= 1.1, fig.cap = "(ref:storyboard-sog-cap)", eval= slides}
create_slide_figures(model_index = model_ind)
```

```{r FSAR-plots-sog, fig.asp= 1.1, eval= TRUE}
...
(ref:eco-diagram-cap) Depiction of some pressures
that can affect different life history 
stages of Pacific Herring.
Diagram copied with permission from @dfo2022c.

(ref:eco-diagram-cap-fr) Représentation de certaines pressions
qui peuvent affecter les différents stades du cycle biologique
du hareng du Pacifique.
Diagramme copié avec l'autorisation de @dfo2022c.

```{r eco-diagram, fig.cap = ifelse(french, "(ref:eco-diagram-cap-fr)", "(ref:eco-diagram-cap)"), out.width = "100%"}
knitr::include_graphics(
  here(ifelse(french, "figures/eco_diagram_fr.png", "figures/eco_diagram.png"))
)
```

<!-- TODO: -->
<!-- Remove horizontal lines and dots (figure and caption) -->
<!-- Remove /m3 and mg/m3 from figure -->
<!-- Define seasons (Spring, Summer, Fall, Winter) -->
<!-- Add "Copied with permission from (Boldt et al.).$^{\ref{fn:boldt}}$" -->
<!-- Add x- and y-axes labels -->

(ref:eco-trend-cap) Selected standardized pressure indicators and 
Pacific Herring responses (e.g., status and trend) in the last five years.
Solid green lines are within one standard deviation (SD);
the most recent five years are highlighted in green.
Symbols on the right indicate trend (top) and status (bottom).
Trend indicates a significant
increase (up arrow) or decrease (down), or
neither (horizontal) in the last five years,
where there are more than two years of data.
Status indicates if recent average values are
greater than one SD above (plus) or below (minus), or
neigher (dot) the time series average.
Note that
pressure indicators are for Hecate Strait and Queen Charlotte Sound, and
responses are for Haida Gwaii (HG) herring.
Legend: sea surface temperature (\link{https://www.avl.class.noaa.gov/release/data_available/avhrr/index.htm}{AVHRR satellite});
prey abundance and biomass are in
$\text{number}/\text{m}^3$ and $\text{mg}/\text{m}^3$, respectively,
where $\text{m}$ is metres and $\text{mg}$ is milligrams
(M. Galbraith and R.I. Perry, Fisheries and Oceans Canada (DFO), pers. comm.);
Humpback Whale abundance
(@best2015 for 2008 estimate,
DFO survey (B. Wright, DFO, pers. comm.) for 2018 estimate, and
other years interpolated using a 4% population growth rate
[@ford2010, T. Doniol-Valcroze, DFO, pers. comm.]);
Steller Sea Lion abundance is interpolated
(S. Tucker, DFO, pers. comm.);
Pacific Hake biomass is from the hake stock assessment and acoustic trawl survey
(S. Johnson, Landmark Fisheries Research, pers. comm.);
Arrowtooth Flounder, Spiny Dogfish, and Pacific Cod
catch per unit effort (CPUE) is from the 
International Pacific Halibut Commission (A. Edwards, DFO, pers. comm.);
median HG herring spawning biomass and
weight in grams of age-3 herring [@dfo2021c]; and
residuals from a double log-transformed length-weight regression
for female HG herring
(Boldt et al.).$^{\ref{fn:boldt}}$

(ref:eco-trend-cap-fr) Indicateurs de pression normalisés sélectionnés et réponses du hareng du Pacifique (par exemple, état et tendance) au cours des cinq dernières années.
Les lignes vertes pleines se situent à l'intérieur d'un écart-type (ET) ;
les cinq années les plus récentes sont surlignées en vert.
Les symboles à droite indiquent la tendance (en haut) et l'état (en bas).
La tendance indique une augmentation (flèche vers le haut) ou une diminution (flèche vers le bas)
significative (flèche vers le haut) ou une baisse (vers le bas), ou
ni l'un ni l'autre (horizontal) au cours des cinq dernières années,
lorsqu'il y a plus de deux années de données.
L'état indique si les valeurs moyennes récentes sont
supérieures à une ET au-dessus (plus) ou au-dessous (moins), ou
supérieures (point) à la moyenne de la série chronologique.
Il convient de noter que
les indicateurs de pression concernent le détroit d'Hécate et le détroit de la Reine-Charlotte, et les
sont pour le hareng de Haida Gwaii (HG).
Légende : température de surface de la mer (\link{https://www.avl.class.noaa.gov/release/data_available/avhrr/index.htm}{satellite AVHRR}) ;
l'abondance et la biomasse des proies sont exprimées en
$\text{number}/\text{m}^3$ et $\text{mg}/\text{m}^3$, respectivement,
où $\text{m}$ représente les mètres et $\text{mg}$ les milligrammes.
(M. Galbraith et R.I. Perry, Pêches et Océans Canada (MPO), comm. pers ;)
Abondance des baleines à bosse
(@best2015 pour l'estimation de 2008,
enquête du MPO (B. Wright, MPO, comm. pers.) pour l'estimation de 2018, et
autres années interpolées à l'aide d'un taux de croissance de la population de 4% [@ford2010, T. Donovan, comm. pers.
(@ford2010, T. Doniol-Valcroze, MPO, comm. pers.]) ;
L'abondance de l'otarie de Steller est interpolée
(S. Tucker, MPO, comm. pers.) ;
La biomasse du merlu du Pacifique provient de l'évaluation des stocks de merlu et de l'étude acoustique au chalut.
(S. Johnson, Landmark Fisheries Research, comm. pers.) ;
Plie à dents de flèche, aiguillat commun et cabillaud du Pacifique
la prise par unité d'effort (CPUE) provient de la 
Commission internationale du flétan du Pacifique (A. Edwards, MPO, comm. pers.) ;
biomasse médiane de harengs reproducteurs HG et
le poids en grammes du hareng d'âge 3 [@dfo2021c] ; et
les résidus d'une régression longueur-poids à double transformation logarithmique
pour le hareng femelle HG
(Boldt et al.).$^{\ref{fn:boldt}}$

```{r eco-trend, fig.cap = ifelse(french, "(ref:eco-trend-cap-fr)", "(ref:eco-trend-cap)"), out.width = "100%"}
knitr::include_graphics(
  here(ifelse(french, "figures/eco_trend.png", "figures/eco_trend.png"))
)
```

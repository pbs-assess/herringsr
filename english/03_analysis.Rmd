...
have drafted for each major SAR, however only the conservation objective
(1) has been used in the selection of MPs:
...
For the remaining 4 SARs, we implement MPs from the previously
...
Incidental mortality is described in @dfo2024 and updated time series for each
SAR can be found in 
\link{https://github.com/grinnellm/Reports}{Pacific Herring data summaries}.
These data are not currently included as removals in Herring stock assessments.

## First Nations observations

Data and observations for the 2024 herring spawning season were contributed by 
representatives of First Nations communities for each SAR and can be found in
the \link{https://github.com/grinnellm/Reports/tree/master/Summaries}{Pacific Herring data summaries}.
This includes contributions from the Haida Nation in Haida Gwaii; 
...
but low estimated recruitment has failed to lead to a positive estimated 
productivity.
...
about 50% of the historic high, but began a steady decline 
in the late-1980s,
...
(Figure \@ref(fig:storyboard-prd)f) and with natural mortality decreasing 
in the last handful of years. This has led to an abundance estimate that is 
steeply increasing, and higher than estimates since the 1960s.
...
Commercial fisheries have occurred annually in PRD from the mid-1980s through 
2018, during which the effective harvest rate $U_t$ was estimated to be
...
Spawning biomass trend declined from 1985-2008 and 
...
Model estimates suggest decreasing spawning biomass since 2020
(Table \@ref(tab:cc-spawning-biomass-depletion)),
and the analysis of surplus production shows
an estimated production close to neutral for 2021 and 2023, and negative for 2023.
...
Upper Spiller Channel (Section 078), Section 086, and
Kitasu Bay/East Higgins (Section 067), Section
...
from 2014 to 2024 (Table \@ref(tab:annual-sok-catch)).
...
down from the peaks observed in the late 1970s to a slump in the mid 2000s to
mid 2010s (Figure \@ref(fig:storyboard-wcvi)a, b, c).
...
(Figure \@ref(fig:storyboard-wcvi)a, b, f). Biomass estimates are nearing the 
peak seen on the WCVI in the over 70 year history of DFO records.
...
<!--TODO (SP): I'm not sure this next statement fits here, as we don't provide this clarification
for the other SARs. Maybe this can be added to the Appendix, under
Model description (eg, Table of Priors) ?? 
Last year because there was so much autocorrelation we provided some verbage.
This year it might be good to show that it was taken care of. Otherwise just 
delete this paragraph-->
Previous years WCVI MCMC diagnostics showed autocorrelation 
especially in the estimation of fishery selectivity-at-50% ($\hat{a}_1$) 
and its standard deviation ($\hat{\gamma}_1$)
for the "other" fisheries category (i.e., reduction, food and bait, as well as 
special use). These were largely ameliorated by a reparameterization of 
parameter priors [@dfo2021c]).
...
and declined from 2021 to 2023, with a slight uptick in 2024.
...
Given it's prolonged low biomass state, a rebuilding plan was required 
for Haida Gwaii Pacific Herring. 
...
from 2021-2024 (Figure \@ref(fig:storyboard-cc)a).
...
application of MPs to the 2024 forecast biomass for CC,
...
Firstly, Pacific Herring in BC are managed as 5 major stocks, taking into account
stock specific life history, abundance, and pressures that feed into the models. 
Secondly, the Limit Reference Point is set to a higher values compared to those
recommended in 
<!--TODO (JC) add wording here. -->
due to mid-trophic level (middle-of-the-food-chain) nature of Herring which 
likely results in the high variability in Herring productivity.
...
Environmental pressures include the timing, duration, and magnitude of upwelling,
which can affect the amount of prey available to 
Herring [@mackas2001; @hourston2019], as well as predation and
competition [@Goedfroid2019].
...
Thirdly, while these ecosystem indicators are not directly incorporated into the 
assessment model they are modelled implicitly via time varying natural mortality 
and recruitment. Incorporating spatial, and other 
ecosystem indicators is an area of ongoing research for Pacific Herring.
DFO is committed to an ecosystem approach to fisheries management and provides 
a summary of recent research in [@dfo2024]. 

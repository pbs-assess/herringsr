...
In 2022, Heiltsuk observed that
the herring season spawning behavior was similar to last year
...
Predation by whales, seals and sea lions has continued to
disrupt and move spawning herring around,
...
Heiltsuk sounding vessels and test boat skippers reported
adequate tonnage in areas with estimates of 32,000 to 36,000 tonnes
...
around Vargas, Stubbs, and Mears (Opisaht) islands.
...
Muscle Beach/Cabbage Rocks to Macoah on March 9 and
there was a spot spawn around Spilling Islets on March 10.
...
This year,
Markov chain Monte Carlo (MCMC) runs have chain length `r n2w(mcmc_length)`
with a sample taken every `r n2w(mcmc_samp_freq)` iterations (i.e., thinning).
The first `r n2w(mcmc_burnin)` samples are discarded (i.e., burn-in),
leaving `r n2w(mcmc_num_samples)` samples for posteriors.
...
There was an increase in survey biomass in 2019 and 2020, and
...
There is large uncertainty for the last few years of spawning biomass and

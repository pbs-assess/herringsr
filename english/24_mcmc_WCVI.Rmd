...
The MCMC run had chain length `r n2w(mcmc_length)`
with a sample taken every `r n2w(mcmc_samp_freq)` iterations (i.e., thinning).
The first `r n2w(mcmc_burnin)` samples were discarded (i.e., burn-in),
...
```{r hcr-wcvi-1, fig.asp=1, fig.cap="(ref:hcr-wcvi-1-cap)", eval=FALSE}
region <- "WCVI"
...
```{r hcr-wcvi-2, fig.asp=1, fig.cap="(ref:hcr-wcvi-2-cap)", eval=FALSE}
region <- "WCVI"
probs <- confidence_vals
cowplot::plot_grid(
  plot_hcr(

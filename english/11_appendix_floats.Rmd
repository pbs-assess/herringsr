...
dat <- read_csv(file = file.path(data_path, "spawn-yr-sec-a27.csv"),
                col_types=cols()) %>%
        filter(Year >= minor_start_yr_plot) %>%
        mutate(Section = as.numeric(Section)) %>%
        mutate(Survey = factor(Survey, levels = c("Surface", "Dive")),
               Section = formatC(Section, width = 3, format = "d", flag = "0"))
ggplot(data = dat, mapping = aes(x = Year, y = Index)) +
        ...
        expand_limits(
                y = c(0, 100), x = c(minor_start_yr_plot, minor_end_yr)
        ) +
        labs(x = en2fr("Year", french),
             y = paste0(en2fr("Spawn index", french), " (1,000 t)"),

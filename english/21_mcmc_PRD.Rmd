...
(ref:hcr-prd-1-cap) Prince Rupert District SAR:
projected spawning biomass for `r assess_yr + 1` $\SB_{`r assess_yr+1`}$
compared to harvest rate (HR) and total allowable catch (TAC;
rows 2 to 7 in Table \@ref(tab:decision-prd)).
Note: management procedures that specify no fishing
are excluded from this figure.
Red horizontal and vertical lines show median HR, TAC,
and $\SB_{`r assess_yr+1`}$;
green lines show mean statistics.

```{r hcr-prd-1, fig.asp=1, fig.cap="(ref:hcr-prd-1-cap)", eval=FALSE}
region <- "PRD"
...
projected spawning biomass for `r assess_yr + 1` $\SB_{`r assess_yr+1`}$
compared to harvest rate (HR) and total allowable catch (TAC;
rows 9 to 14 in Table \@ref(tab:decision-prd)).
See Figure \@ref(fig:hcr-prd-1) for description.

```{r hcr-prd-2, fig.asp=1, fig.cap="(ref:hcr-prd-2-cap)", eval=FALSE}
region <- "PRD"
probs <- confidence_vals
cowplot::plot_grid(
  plot_hcr(

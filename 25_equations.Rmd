...
$kth$ survey the vulnerable biomass available to the survey sampling gear:
...
q_k = \frac{1}{N_k}\sum_{tEI_{k,t}} ln(I_{k,t}) - ln(V_{k,t})
\end{equation}

where $N_k$ is the number of relative abundance observations for index $k$ (see Walters and Ludwig [citation],
1994, for more information). The negative loglikelihood for relative abundance data is given
by:

\begin{equation}\label{eq:l_I}
\ell_I = \sum_{k}\sum_{tEI_{k,t}} ln(\alpha_{k,t}) + \frac{\epsilon_{k,t}^2}{2\alpha_{k,t}^2}
\end{equation}
...
\eta_{t,a} = ln(p_{t,a}) - ln(\hat{p}_{t,a})-\frac{1}{A}
\sum^{A}_{a=1}[ln(p_{t,a}) - ln(\hat{p}_{t,a})] 
\end{equation}
...
\hat{\tau}^2=\frac{1}{(A-1)T}\sum^{T}_{t=1}\sum^{A}_{a=1}\eta_{t,a}^2
\end{equation}
...
\delta_{t} = ln(\bar{R}e^{\omega_{t}}) - R_{t}
\end{equation}
...
\ell_{\delta} = nln(\tau) +\frac{\sum^{T}_{t=1} +k^{\delta_{t}^2}}{2\tau^2}
\end{equation}
...
\text{priors for parameters} + \text{survey priors} +

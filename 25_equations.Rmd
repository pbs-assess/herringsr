...
The Pacific Herring stock assessment is conducted using the Integrated Statistical Catch Age Model (ISCAM), developed by S. Martell (Martell et al., 2012). 
ISCAM uses a statistical catch-at-age model implemented in a Bayesian estimation framework and is written in AD Model Builder. The original source code and documentation for both are available online. 
...
The documentation presented here is a revised version of the ISCAM user guide available online. Much of the text and many of the
...
The model documentation describes all features of the ISCAM catch-age model, some of which however
are not implemented for the Pacific Herring stock assessment. 
...
5. Unfished spawning biomass is represented as B0 in the Model Description, and as SB0 in
the main text.
...
\begin{equation}\label{eq:Theta_steady}
\Theta = \left(R_0,\,h,\,M\right)
\end{equation}
is unknown, and can be estimated by fitting the SCA model to data with the following restrictions:
$R_0 > 0$, $0.2 \leq h < 1.0$, and $M > 0$.
...
The observed length at age l_{a,s} is modeled using the Von-Bertalanffy growth model:
\begin{equation}\label{eq:l_as}
l_{a,s} = l\left(1-e^{\left(-k_s\left(a-t_{0,s}\right)\right)}\right),
\end{equation}
mean weight-at-age is given by the allometric relationship
\begin{equation}\label{eq:w_as}
...
\begin{equation}\label{eq:iota_a_1}
\iota_a = \begin{cases}
\frac{1}{S}, & a=1\\
\iota_{a-1}e^{-M}, & 1<a<A\\
...
\begin{equation}\label{eq:iota_a_2}
\iota_a = \begin{cases}
\frac{1}{S}, & a=1\\
...
\begin{equation}\label{eq:Theta_catch}

...
The scaling parameter $q$ for each of the surveys is not treated as an unknown parameter within the code;
rather, the maximum posterior density estimate for $\lognorm(q)$ conditional on all other parameters is used to scale the predicted spawning biomass to the observed 
acoustic biomass index.
For the AM2 version of the Pacific Herring assessment model used in this analysis,
$q_1$ is assumed to have an uninformative prior
$q_1\sim$\dUnif{0}{1}, and 
$q_2$ is assumed to have an informative prior
$q_2\sim$\dNorm{1}{0.01}. 

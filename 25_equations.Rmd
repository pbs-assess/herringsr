...
$\Theta$, is assumed to be unknown,

\begin{equation}\label{eq:Theta-steady}
\Theta = \left(R_0,\,h,\,M\right)
\end{equation}

and can be estimated by fitting the SCA model to 
data with the following restrictions:
...
Selectivity parameters can be fixed or estimated. Fecundity-at-age is defined in terms of weight-at-age and age-based logistic function: 
...
Unfished spawning biomass is calculated by,
...
Recruitment is assumed to follow a Beverton-Holt stock recruitment relationship,
...
These equations can then be simplified to the steady-state equilibrium recruitment:
  \begin{equation}\label{eq:R-e}
R_e = R_0 \frac{\kappa-\frac{\phi_E}{\phi_e}}{\kappa-1}  
\end{equation}

The equilibrium yield $C_e$ for a given fishing mortality rate, $F_e$ is given by:
...
The estimated parameter vector in the dynamic state version of iSCAM is defined by $\Theta$,
where $R_0$, $h$, and $M$, are the leading population parameters that define the overall scale and productivity of the population. The variance components of the model are partitioned using an errors in variables approach. 
...
which represents the proportion of the total variance that is due to observation error(Punt and Butterworth, 1999; Deriso et al., 2007).
...
The unobserved state variables include the numbers-at-age each year, $N_{t,a,s}$, the total
biomass each year, $B_{t,s}$, the spawning stock biomass $SB_{t,s}$ and the total age-specific total mortality
rate (Zt;a). The initial numbers-at-age in the first year (Eq. A.31) and the annual recruits
(Eq. A.32) are treated as estimated parameters and used to initialize the numbers-at-age
array.

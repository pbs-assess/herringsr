...
which describes the observed length-at-age $l_{a,s}$ (Equation \ref{eq:l-as}).
The mean weight-at-age $w_{a,s}$ is given by an allometric relationship (Equation \ref{eq:w-as}), and
...
\Theta = \left(R_0,\,h,\,M,\,\overline{R},\,\overline{R}_{\mli{init}},\,\vartheta^2,\,\rho,\,               \Gamma_{k,t},\,\{\omega_t\}_{\acute{t}=1-A}^{\acute{t}=T},\,               \{\omega_{\mli{init},t}\}_{t=\acute{t}-A}^{t=\acute{t}-1}\right)
\end{equation}
...
include numbers-at-age each year $N_{t,a,s}$,
...
The total mortality rate is given by Equation \@ref(eq:Z-tas-state),
and the total catch (in weight) for each gear is given by Equation \@ref(eq:C-kt),
assuming that both natural and fishing mortality occur simultaneously throughout the year.
The sex-specifc numbers-at-age are propagated over time using 
(Equation \ref{eq:N-tas-state}), where members of the plus group
(age $A$) are all assumed to have the same total mortality rate $Z_{t,a,s}$. 
...
(where $k$ in this case refers to the start age, which for Pacific Herring is age 2)
...
1. Three penalty functions that are invoked to regularize the solution during intermediate
...
prior probability distributions were the most important contributors to the objective function.
...
where $o$ is a small constant ($e^{-10}$) to
ensure the residual is defined in the case of a zero catch observation.
The residuals are assumed to be normally distributed with a user-specified standard deviation $\sigma_C$.
...
For Pacific Herring the relative abundance data are assumed to be proportional to spawning biomass
so the $k^\text{th}$ survey the vulnerable biomass available to the survey sampling gear:
...
associated with observation errors, and
...
set $\omega_{k,t} = 2.0$.
To assume all observations have the same variance then simply
set $\omega_{k,t} = 1$. 
...
the spawn survey data since `r new_surv_yr` are assumed to be 1.166 times as precise as
...
The standard deviation in the observation errors is conditional on estimated values of $\rho$ and $\varphi^2$.
...
age-proportion data can be weighted based on the conditional maximum likelihood estimate of the variance in the age-proportions.
Therefore, the contribution of the age-composition data to the overall objective function is “self-weighting” and is conditional on other componentsin the model.
...
and residuals between estimated recruits and the deterministic stock-recruitment models are used to
...
where $R_t$ is given by Eq uation \@ref(eq:R-t), and 
...
Equations \@ref(eq:delta-t) and \@ref(eq:l-delta) are key for estimating unfished spawning stock biomass and
recruitment compensation via the recruitment models. 
The relationship between ($\alpha$, $\beta$) and
($B_0$, $\kappa$) is defined by Equations \@ref(eq:alpha) and \@ref(eq:beta)
Where again $\alpha$ is the maximum juvenile 
survival rate,
$\beta$ is the density effect on recruitment, 
and $B_0$ is the unfished spawning stock biomass.
...
This subtle calculation has implications for reference point calculations; in cases where there are increasing or decreasing trends in natural mortality rates trend upwards, estimates of $B_0$ decrease.
...
All estimated selectivity parameters $\vec{\gamma_k}$ are estimated in log
space and
...
Log annual recruitment deviations are also constrained to have minimum and maximum values of -15.0 and 15.0
and there is an additional constraint to ensure the vector of deviations sums to 0.
...
can also specify an informative prior distribution for each of these parameters. 
...
For Pacific Herring, a bounded uniform prior was specified for the 
log of unfished recruitment \dUnif{-5}{15},
a beta prior was assumed for steepness \dBeta{10.0}{4.92},
a normal prior was specified for the log of natural mortality rate 
\dNorm{-0.79}{0.4},

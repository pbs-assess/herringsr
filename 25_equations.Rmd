...
For the steady-state conditions represented below, we assume the parameter vector Theta in Eq. 1 is unknown and would be estimated by fitting ISCAM to data.
For a given set of growth parameters and maturity-at-age parameters defined by Phi in Eq. 2, 
growth is assumed to follow von Bertalanffy (Eq. 3),
mean weight-at-age is given by the allometric relationship in Eq. 4, and
the age-specific vulnerability is given by an age-based logistic function (Eq. 5).
...
Note that we assume that recruitment follows a Beverton-Holt stock recruitment model of the form shown in Eq. 14,
where the maximum juvenile survival rate so is given by:
...
where $R_0 > 0$, $0.2 \leq h < 1.0$, and $M > 0$.  
...
l_{a,s} = l\left(1-e^{\left(-k_s\left(a-t_{o,s}\right)\right)}\right)
\end{equation}
...
N_{t,a,s} = 
\begin{cases}
\frac{s_0E_{t-1}}{1+\beta E_{t-1}} e^{\left(\omega_t-0.5\tau^2\right)}, & a=1 \\
...
R_t = \frac{s_0\mli{SB}_{t-k}}{1+\beta \mli{SB}_{t-k}}e^{\delta_{t}-0.5\tau^2}
\end{equation}

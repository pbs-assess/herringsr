...
[iSCAM](https://github.com/smartell/iSCAM) and
[ADMB](http://www.admb-project.org/) are available online.

This section contains the documentation in mathematical form of the underlying iSCAM age-structured model,
...
the components of the objective function that formulate the statistical criterion used to estimate model parameters.
...
The documentation presented here is a revised version of the [iSCAM users guide](https://sites.google.com/site/iscamproject/).
Much of the text and many of the equations have been taken directly from this guide.
...
1. Unfished spawning biomass is represented as $B_0$ in this section, and as $\mli{SB}_0$ in the main text.
...
Under steady state conditions the parameter vector $\Theta$ is assumed to be unknown,
...
For a given set of growth and maturity-at-age parameters $\Phi$ (Equation \ref{eq:Phi}),
...
Mean weight-at-age $w_{a,s}$ is given by an allometric relationship (Equation \ref{eq:w-as}), and
age-specific vulnerability $v_a$ is given by an age-based logistic function (Equation \ref{eq:v-a}).
...
the recruitment compensation ratio $\kappa$ [@myers1999] are estimated by
...
Recruitment is assumed to follow a Beverton-Holt stock recruitment relationship, where
...
iSCAM calculates $F_\text{MSY}$ by finding the value of $F_e$ that results in the zero derivative of Equation \@ref(eq:C-e).
...
However, these reference points are not currently used in the Pacific Herring assessment.

## Dynamic state: statistical catch-age model

The estimated parameter vector in the dynamic state version of iSCAM is defined by $\Theta$, where
$R_0$, $h$, and $M$ are the leading population parameters that define the overall scale and productivity of the population

\begin{equation}\label{eq:Theta-catch}
\Theta = \left(R_0,\,h,\,M,\,\overline{R},\,\overline{R}_{\mli{init}},\,
...
This parameter can be fixed or estimated, and is fixed for the Pacific Herring model.
The total variance is partitioned into observation $\sigma$ (Equation \ref{eq:sigma}) and process error $\tau$ (Equation \ref{eq:tau}) components by the model parameter $\rho$,
which represents the proportion of total variance due to observation error [@punt1999; @deriso2007]:
...
<!-- total age-specific total mortality rate $Z_{t,a,s}$. -->
...
Annual fishing mortality for each gear $k$ in year $t$ is the exponent of the estimated vector $\Gamma_{k,t}$ (Equation \ref{eq:F-kt}).
...
where the spawning biomass is the product of numbers-at-age and mature biomass-at-age 
(Equation \ref{eq:B-ts-state}).
Total mortality rate is given by Equation \@ref(eq:Z-tas-state),
and total catch (in weight) for each gear is given by Equation \@ref(eq:C-kt),
assuming that both natural and fishing mortality occur simultaneously throughout the year.
Sex-specifc numbers-at-age are propagated over time using Equation \@ref(eq:N-tas-state),
where members of the plus group (i.e., age $A$) are all assumed to have the same total mortality rate $Z_{t,a,s}$:
...
Recruitment to age $k$ (where $k$ in this case refers to the start age, which for Pacific Herring is age 2)
is assumed to follow a Beverton-Holt model for Pacific Herring (Equation \ref{eq:R-t}), where
the maximum juvenile survival rate $\alpha$ is defined by Equation \@ref(eq:alpha) and
...
1. Three penalty functions that are invoked to regularize the solution during intermediate phases of the non-linear parameter estimation. 

The penalty functions:

1. Constrain the estimates of annual recruitment to conform to a Beverton-Holt stock-recruit function,

1. Weakly constrain the log recruitment deviations to a normal distribution, and

1. Weakly constrain estimates of log fishing mortality to a normal distribution
($\sim$ $N$(ln(0:2); 4:0)) to prevent estimates of catch from exceeding estimated biomass.

Sensitivity analysis indicate that the model is insensitive to changes in the penalty function parameters,
indicating that the other likelihood components and prior probability distributions were the most important contributors to the objective function.
The objective function components are discussed in more detail in the following sections.

### Observed catch

It is assumed that measurement errors in catch observations are log-normally distributed, and the residuals given by:

\begin{equation}\label{eq:ets_kt}
\eta_{k,t} = \lognorm\left(C_{k,t} + o\right) - \lognorm\left(\hat{C}_{k,t} + o\right)
\end{equation}

where $o$ is a small constant (i.e., $e^{-10}$) to
ensure the residual is defined in the case of a zero catch observation.
Residuals are assumed to be normally distributed with a user-specified standard deviation $\sigma_C$.
...
For Pacific Herring, relative abundance data are assumed to be proportional to spawning biomass.
Thus, in the $k^\text{th}$ survey, the vulnerable biomass available to the survey sampling gear
...
\epsilon_{k,t} = \lognorm\left(I_{k,t}\right) - \lognorm\left(q_k\right)+\lognorm\left(V_{k,t}\right)
\end{equation}
...
q_k = \frac{1}{N_k}\sum_{t \in I_{k,t}} \lognorm\left(I_{k,t}\right) - \lognorm\left(V_{k,t}\right)
\end{equation}
...
\ell_I = \sum_k \sum_{t \in I_{k,t}} \lognorm\left(\sigma_{k,t}\right)+\frac{\epsilon_{k,t}^2}{2\sigma_{k,t}^2}
\end{equation}
...
where $\rho\varphi^2$ is the proportion of the total error that is associated with observation errors, and
$\omega_{k,t}$ is a user specified relative weight for observation $t$ from gear $k$.
The $\omega_{k,t}$ allow each observation to be weighted relative to the total error $\rho\varphi^2$.
...
set $\omega_{k,t} = 1$. 
Note that if $\omega_{k,t} = 0$ then Equation \ref{eq:ell_I} is undefined; 
therefore, iSCAM adds a small constant (i.e., $e^{-10}$) to $\omega_{k,t}$, 
which is equivalent to assuming an extremely large variance to ensure the likelihood can be evaluated.

For the Pacific Herring assessment, 
spawn survey data since `r new_surv_yr` are assumed to be 1.166 times as precise as the pre-dive survey data (`r major_start_yr` to `r new_surv_yr-1`). 
To implement this,
objective function weights for the `r major_start_yr` to `r new_surv_yr-1` data were set equal to unity,
and the contemporary data was assigned a relative weight of 1.166. 
The standard deviation in observation errors is conditional on estimated values of $\rho$ and $\varphi^2$.

### Age composition

Sampling theory suggest that age composition data are derived from a multinomial distribution [@fournier1982]. 
However, iSCAM assumes that age-proportions are obtained from a multivariate logistic distribution [@schnute1995; @richards1997].
iSCAM departs from the traditional multinomial model due to choices regarding weighting of age-composition data in the objective function. 
First, the multinomial distribution requires the specification of an effective sample size. 
This weighting may be done arbitrarily or through iterative re-weighting [@mcallister1997; @gavaris2002],
and in the case of multiple and potentially conflicting age-proportions this procedure may fail to converge properly.
...
age-proportion data can be weighted based on the conditional maximum likelihood estimate of the variance in age proportions.
Therefore, the contribution of age-composition data to the overall objective function is “self-weighting” and is conditional on other componentsin the model.
Ignoring the subscript for gear type for clarity,
the observed and predicted proportions-at-age must satisfy the constraint for each year
...
\eta_{t,a} = \lognorm\left(p_{t,a}\right) - \lognorm\left(\hat{p}_{t,a}\right)-\frac{1}{A}
\sum^{A}_{a=1}\left[\lognorm\left(p_{t,a}\right) - \lognorm\left(\hat{p}_{t,a}\right)\right].
\end{equation}
...
The negative loglikelihood evaluated at the conditional maximum likelihood estimate of the variance is given by

\begin{equation}\label{eq:ell_A}
\ell_A = \left(A-1\right)T \lognorm\left(\hat{\tau}^2\right).
\end{equation}

In short, the multivariate logistic likelihood for age-composition data is the
log of the residual variance weighted by the number observations over years and ages.

### Stock recruitment

The Pacific Herring stock assessment assumes Beverton-Holt recruitment. 
Annual recruitment and the initial age-composition are treated as latent variables in iSCAM, and
residuals between estimated recruits and the deterministic stock-recruitment models are used to
...
Note that a bias correction term for the lognormal process errors is included in Equation \@ref(eq:R-t).
The negative loglikelihood for the recruitment deviations is given by the normal density (ignoring the scaling constant)
  
\begin{equation}\label{eq:l-delta}
\ell_{\delta} = nln\left(\tau\right) +\frac{\sum^{T}_{t=1} +k^{\delta_{t}^2}}{2\tau^2}.
\end{equation}

Equations \@ref(eq:delta-t) and \@ref(eq:l-delta) are key for estimating unfished spawning stock biomass and recruitment compensation via the recruitment models. 
The relationship between ($\alpha$, $\beta$) and ($B_0$, $\kappa$) is defined by 
...
Unfished steady-state spawning stock biomass per recruit is given by $\phi_E$ (Equation \ref{eq:phi-E}), 
which is the sum of products between age-specific survivorship and relative fecundity.
...
in cases where there are increasing or decreasing trends in natural mortality rates trend upwards, estimates of $B_0$ decrease.

## Parameter estimation and uncertainty

There are five distinct components that make up the objective function $f$ that is being minimized by iSCAM
...
The negative loglikelihoods pertain specifically to elements that deal with the data and variance partitioning,
...
and iSCAM attempts to estimate model parameters by minimizing the sum of these elements.
...
(i.e., $ln(R_0)$, $h$, $ln(M)$, $ln(\bar{R})$, $ln(\ddot{R})$, $\rho$, and $\vartheta$). 

All estimated selectivity parameters $\vec{\gamma_k}$ are estimated in log space and
have a minimum and maximum values of -5.0 and 5.0, respectively.
These values are hard-wired into the code, but should be sufficiently large/small enough to capture a wide range of selectivities.
...
Log annual recruitment deviations are also constrained to have minimum and maximum values of -15.0 and 15.0 and
...
Finally, annual log deviations in natural mortality rates are constrained to lie between -5.0 and 5.0.

#### Parameter priors

Each of the seven leading parameters specified in the control file
(i.e., $ln(R_0)$, $h$, $ln(M)$, $ln(\bar{R})$, $ln(\ddot{R})$, $\rho$, and $\vartheta$)
are declared as bounded parameters, and in addition users can also specify an informative prior distribution for each of these parameters. 
Five distinct prior distributions can be implemented:

<!-- TODO: a, b, and \sigma appear elsewhere; find different parameters? -->
1. Uniform \dUnif{a}{b}, where $a$ and $b$ are the minima and maxima, respectively,

1. Normal \dNorm{\mu}{\sigma^2}, where $\mu$ and $\sigma^2$ are the mean and variance, respectively,

1. Lognormal \dLognorm{\mu}{\sigma^2}, where $\mu$ and $\sigma^2$ are the mean and variance, respectively,
...
For Pacific Herring,
a bounded uniform prior was specified for the log of unfished recruitment \dUnif{-5}{15},
a beta prior was assumed for steepness \dBeta{10.0}{4.92},
a normal prior was specified for the log of natural mortality rate \dNorm{-0.79}{0.4},
a bounded uniform prior for both the log of initial recruitment and average recruitment \dUnif{-5.0}{15.0},
a beta prior for the variance partitioning parameter $\rho\sim$\dBeta{17:086}{39:0559}, and
a gamma prior for the inverse total standard deviation parameter $\vartheta\sim$\dGamma{25}{28:75}.
The spawn survey scaling parameter $q$ for each survey is not treated as an unknown parameter within the code;
rather, the maximum posterior density estimate for $\lognorm(q)$ conditional on all other parameters scales the predicted spawning biomass to the spawn index.
For the AM2 version of the Pacific Herring assessment model used in this analysis,

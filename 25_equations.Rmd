...
<!-- TODO: a and b appear elsewhere; find different parameters? -->
1) Uniform \dUnif{a}{b}, where $a$ and $b$ are the minima and maxima, respectively,

<!-- TODO: \sigma appears elsewhere; find a different parameter? -->
2) Normal \dNorm{\mu}{\sigma^2}, where $\mu$ and $\sigma^2$ are the mean and variance, respectively,

3) Lognormal \dLognorm{\mu}{\sigma^2}, where $\mu$ and $\sigma^2$ are the mean and variance, respectively,

<!-- TODO: a and b appear elsewhere; find different parameters? -->
4) Beta \dBeta{a}{b}, where $a$ and $b$ are the shape parameters, and 

5) Gamma \dGamma{a}{b}, where $a$ and $b$ are the shape parameters.

For Pacific Herring, a bounded uniform prior was specified for the 
log of unfished recruitment \dUnif{-5}{15},
a beta prior was assumed for steepness $\emph{beta}(10.0, 4.92)$,
a normal prior was specified for the log of natural mortality rate 
$\mathcal{N}(-0.79, 0.4)$, a bounded uniform prior for both the log 
of initial recruitment and average recruitment $\emph{U}(-5.0, 15.0)$,
a beta prior for the variance partitioning parameter 
$\rho \sim \emph{Beta}(17:086; 39:0559)$, and a gamma prior for the 
inverse total standard deviation parameter $\vartheta \sim \emph{Gamma}(25; 28:75)$.
...
used to scale the predicted spawning biomass to the observed 
acoustic biomass index. For the AM2 version of the Pacific Herring 
assessment model the prior on $q_1$ was assumed uninformative 
$q_1 \sim \mathcal{N}(0, 1)$, and informative for $q_2 \sim \mathcal{N}(1;0:01)$. 

...
to follow a Beverton-Holt model [citation] for Pacific Herring 
...
\begin{equation}\label{eq:lc_kt}
\ell_{C} = \sum_{k}\left[T_k \text{ln}\left(\sigma_{C}\right)+
\frac{\sum_{t}\left(\eta_{k,t}\right)^2}{2\sigma_{C}^2}\right]
\end{equation}
...
q_k = \frac{1}{N_k}\sum_{tEI_{k,t}} \text{ln}\left(I_{k,t}\right) - \text{ln}\left(V_{k,t}\right)
\end{equation}

where $N_k$ is the number of relative abundance observations for index $k$ (see Walters and Ludwig [citation], 1994, for more information).
The negative loglikelihood for relative abundance data is given by:

\begin{equation}\label{eq:l_I}
\ell_I = \sum_{k}\sum_{tEI_{k,t}} \text{ln}\left(\alpha_{k,t}\right) + \frac{\epsilon_{k,t}^2}{2\alpha_{k,t}^2}
\end{equation}

The standard deviation $\alpha_{k,t}$ is calculated by:

\begin{equation}\label{eq:alpha_kt}
\alpha_{k,t} = \frac{\rho\varphi^2}{\omega_{k,t}}  
\end{equation}
...
Note that if $\omega_{k,t}$ = 0 then Eq. \ref{eq:l_I} is undefined; 
therefore, iSCAM adds a small constant to $\omega_{k,t}$ 
($e^{-10}$, which is equivalent to assuming an extremely large variance) 
...
derived from a multinomial distribution (Fournier and Archibald, 1982)[citation]. 
However, iSCAM assumes that age-proportions are obtained
from a multivariate logistic distribution (Schnute and Richards, 1995; Richards et al., 1997)[citation].
...
re-weighting (McAllister and Ianelli, 1997; Gavaris and Ianelli, 2002), [citation]
...
\ell_{A} = \left(A-1\right)T \text{ln}\left(\hat{\tau}^2\right)
\end{equation}
...
Parameter estimation and quantifying uncertainty was carried out 
using the tools available in AD Model Builder. AD Model Builder (ADMB) is a software for creating computer programs to estimate the parameters and associated probability distributions for nonlinear statistical models.
The software is freely available from the ADMB project. This software was used to develop
iSCAM, and the source code and documentation for the original version of iSCAM (on which iSCAM is based) is freely available from the iSCAM project, or from a subversion repository on
GitHub.

There are actually five distinct components that make up the objective function $f$ that ADMB is minimizing:
...
To reiterate, these are the likelihood of the catch data $l_C$, 
likelihood of the survey data $l_I$, 
the likelihood of the age-composition data $l_A$, 
and the likelihood of the stock-recruitment residuals $l_\delta$.
...
($ln(R_0)$, $h$, $ln(M)$, $ln(\bar{R})$, $R$, $\rho$, $\vartheta$). 

...
State variables in each year are updated using (Eqns. \ref{eq:B-ts-state} - \ref{eq:N-tas-state}), where the spawning biomass is the product of the numbers-at-age and the mature biomass-at-age (Eq. \ref{eq:B-ts-state}). The total mortality rate is given by (Eq. \ref{eq:Z-tas-state}), and the total catch (in weight) for each gear is given by (Eq. \ref{eq:C-kt}), assuming that both natural and fishing mortality occur simultaneously throughout the year. The sex-specifc numbers-at-age are propagated over time using (Eq. \ref{eq:N-tas-state}), where members of the plus group (age A) are all assumed to have the same total mortality rate $Z_{t,a,s}$. Recruitment to age k is assumed to follow a Beverton-Holt model for Pacific herring (Eq. \ref{eq:R-t}) where the maximum juvenile survival rate $\alpha$ is defined by (Eq. \ref{eq:Alpha}) and $\beta$ is derived from (Eq. \ref{eq:beta}) which is conditional on estimates of $h$ and $R_{o}$.

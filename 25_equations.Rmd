...
Survivorship for unfished and fished populations is defined by \@ref(eq:iota_a_1) and \@ref(eq:iota_a_2), respectively.
It is assumed that all individuals ages $A$ and older (i.e., the plus group) have the same total mortality rate.
The incidence functions refer to the life-time or per-recruit quantities such as spawning biomass per recruit ($\phi_E$; \@ref(eq:phi_E)) or vulnerable biomass per recruit ($\phi_B$, \@ref{eq:phi_B}).
...
The steady-state equilibrium recruitment for a given fishing mortality rate $F_e$ is given by Eq. 13.

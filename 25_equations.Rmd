...
\eta_{k,t} = \text{ln}\left(C_{k,t} + o\right) - \text{ln}\left(\hat{C}_{k,t} + o\right)
\end{equation}
...
<!-- TODO: Use variables that don't appear elsewhere (alpha, beta, sigma) -->
Five distinct prior distributions can be implemented:
uniform $\mathcal{U}(\text{min}, \text{max})$ where $\text{min}$ and $\text{max}$ are the minima and maxima, respectively,
normal $\mathcal{N}(\mu, \sigma)$ where $\mu$ and $\sigma$ are the mean and standard deviation, respectively,
lognormal $\mathcal{L}(\mu, \sigma)$ where $\mu$ and $\sigma$ are the mean and standard deviation, respectively,
beta $\mathcal{B}(\alpha, \beta)$ where $\alpha$ and $\beta$ are the shape parameters, and
gamma $\mathcal{G}(\alpha, \beta)$ where $\alpha$ and $\beta$ are the shape parameters.
For Pacific Herring,

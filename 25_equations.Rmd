...
\Theta = (R_o,h,M); \enspace
R_o > 0, \enspace
0.2 \leq h < 1.0, \enspace
M > 0  
\end{equation}

\begin{equation}\label{eq:df2}
\Phi = (l_{\infty,s},\acute{k}_s,
t_{o,s},\acute{a}_s,\acute{b}_s,\dot{a}_s,\dot{\gamma}_s,\hat{a}_k,\hat{\gamma}_k)
\end{equation}
...
\phi_E = \sum_{s=1}^S\sum_{a=1}^\infty \iota_a f_{a,s}, \enspace
\phi_e = \sum_{s=1}^S\sum_{a=1}^\infty \hat{\iota}_{a,s} f_{a,s}
\end{equation}

\begin{equation}\label{eq:df10}
\phi_B = \sum_{s=1}^S\sum_{a=1}^\infty \iota_a w_{a,s} v_{a,s}, \enspace
...
\Theta = (R_0,h,M,\overline{R},\overline{R}_{init},\vartheta^2,\rho,
\Gamma_{k,t},\{\omega_t\}_{\acute{t}=1-A}^{\acute{t}=T},
\{\omega_{init,t}\}_{t=\acute{t}-A}^{t=\acute{t}-1})
\end{equation}

\begin{equation}\label{eq:df17}
\sigma = \sqrt{\rho}\vartheta, \enspace
\tau = \sqrt{(1-\rho)}\vartheta  
\end{equation}
...
N_{t,a,s} = \frac{1}{S}\overline{R}_{init}e^{\omega_{init,t}}e^{-M(a-1)}; \enspace
(\acute{t}-A)<t<1; \enspace
2 \leq a \leq A   
\end{equation}
 
\begin{equation}\label{eq:df20}
N_{t,a,s} = \frac{1}{S}\bar{R}e^{\omega_{t}}; \enspace
1 \leq t \leq T, \enspace
a = 1  
\end{equation}
...
\hat{C}_{k,t} = \sum_s \sum_a \frac {N_{{t,a,s}}w_{{a,s}}F_{k,t} v_{{k,t,a,s}} ( 1-{e^{-Z_{t,a,s}}} ) }{Z_{t,a,s}}^{\eta_t}
\end{equation}

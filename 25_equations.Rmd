...
\begin{table}[!h]
\centering
\caption{\label{tab:steady-state-model} Steady-state age-structured model assuming unequal vulnerability-at-age, age-specific fecundity, and Beverton-Holt recruitment.}
\begin{tabular}{l}
\hline \\
...
%% \vbox{\begin{equation} R_e=R_o \dfrac{\ln(\kappa)-\ln(\phi_E/\phi_e)}{\ln(\kappa)}\ \ \ (Ricker) \label{eq:df13} \end{equation}} \\
...
\\
\vbox{\begin{equation}  Z_{t,a,s}=M+\sum_k F_{k,t} v_{k,t,a,s}  \label{eq:df24} \end{equation}} \\[2ex]
\\
...
\\
\\
\vbox{\begin{equation} R_t = \frac{s_oB_{t-k}}{1+\beta B_{t-k}}e^{\delta_{t}-0.5\tau^2}\ \ \ (Beverton-Holt) \label{eq:df27} \end{equation}} \\
\end{tabular}
\end{table}
\FloatBarrier

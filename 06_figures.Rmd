...
Panel (a): model fit (maximum posterior density, MPD; lines) to scaled spawn survey data (points).
...
Panel (d): posterior estimate of spawning biomass $\mli{SB_t}$ in year $t$.
Circle and vertical line indicate the median and `r ci_level*100`% credible interval, respectively, of forecast spawning biomass in `r assess_yr+1` in the absence of fishing.
...
Panel (f): phase plot of spawning biomass production ($\frac{\mli{SB}_{t+1} - \mli{SB}_t + C_{t+1}}{\mli{SB}_t}$) for the dive survey period (`r new_surv_yr` to `r assess_yr-1`; MPD estimates).
Grey shading becomes darker in chronological order; the triangle indicates `r assess_yr-1`.
Panels (d & f): red lines and shading indicate medians and `r ci_level*100`% confidence intervals, respectively, for the limit reference point $0.3\mli{SB}_0$, where $\mli{SB}_0$ is estimated unfished spawning biomass.
Note: biomass and catch are in thousands of tonnes (t).

(ref:storyboard-hg-cap-french) Production d’un modèle pour le hareng du Pacifique dans la région d’évaluation des stocks majeur de Haida Gwaii.
Panneau (a): ajustement du modèle aux données mises à l’échelle de relevé du frai en milliers de tonnes métriques.
Les données de relevé du frai (l’indice du frai) sont mises à l’échelle de l’abondance à l’aide du paramètre de mise à l’échelle du relevé du frai $q$.
Panneau (b): estimations a posteriori du taux de mortalité naturelle instantanée.
Panneau (c): nombre reconstitué de recrues d’âge 2 en milliers de millions.
Panneau (d): estimation a posteriori de la biomasse reproductrice $\mli{BR_t}$ pour chaque année $t$ en milliers de tonnes.
...
Panneaux (c et e): les cercles et les lignes verticales indiquent les médianes et les intervalles de crédibilité de `r ci_level*100`%, respectivement.
Panneau (f): diagramme de phase de la production de la biomasse reproductrice ($\frac{\mli{BR}_{t+1} - \mli{BR}_t + C_{t+1}}{\mli{BR}_t}$) pour la période du relevé de plongée (de `r new_surv_yr` à `r assess_yr-1`; estimations de la densité a posteriori maximale).
...
Panneaux (d et f): les lignes rouges et l’ombrage indiquent les médianes et les intervalles de confiance de `r ci_level*100`%, respectivement, pour le point de référence limite $0,3\mli{BR}_0$, où $\mli{BR}_0$ est la biomasse reproductrice non exploitée estimée.

```{r storyboard-hg, out.height="6.4in", fig.asp=1, fig.cap = ifelse(french, "(ref:storyboard-hg-cap-french)", "(ref:storyboard-hg-cap)")}
...
```
<!-- Prince Rupert District Storyboard -->
(ref:storyboard-prd-cap) Model output for Pacific Herring in the Prince Rupert District major stock assessment region.
See Figure \@ref(fig:storyboard-hg) for description.

(ref:storyboard-prd-cap-french) Production d’un modèle pour le hareng du Pacifique dans la région d’évaluation des stocks majeur du district de Prince Rupert.
Voir la figure \@ref(fig:storyboard-hg) pour la description.

```{r storyboard-prd, out.height="6.4in", fig.asp=1, fig.cap = ifelse(french, "(ref:storyboard-prd-cap-french)", "(ref:storyboard-prd-cap)")}
...
```
<!-- Central Coast Storyboard -->
(ref:storyboard-cc-cap) Model output for Pacific Herring in the Central Coast major stock assessment region.
See Figure \@ref(fig:storyboard-hg) for description.

(ref:storyboard-cc-cap-french) Production d’un modèle pour le hareng du Pacifique dans la région d’évaluation des stocks majeur de la côte centrale.
Voir la figure \@ref(fig:storyboard-hg) pour la description.

```{r storyboard-cc, out.height="6.4in", fig.asp=1, fig.cap = ifelse(french, "(ref:storyboard-cc-cap-french)", "(ref:storyboard-cc-cap)")}
...
```
<!-- Strait of Georgia Storyboard -->
(ref:storyboard-sog-cap) Model output for Pacific Herring in the Strait of Georgia major stock assessment region.
See Figure \@ref(fig:storyboard-hg) for description.

(ref:storyboard-sog-cap-french) Production d’un modèle pour le hareng du Pacifique dans la région d’évaluation des stocks majeur du détroit de Georgie.
Voir la figure \@ref(fig:storyboard-hg) pour la description.

```{r storyboard-sog, out.height="6.4in", fig.asp=1, fig.cap = ifelse(french, "(ref:storyboard-sog-cap-french)", "(ref:storyboard-sog-cap)")}
...
```
<!-- West Coast of Vancouver Island Storyboard -->
(ref:storyboard-wcvi-cap) Model output for Pacific Herring in the West Coast of Vancouver Island major stock assessment region.
See Figure \@ref(fig:storyboard-hg) for description.

(ref:storyboard-wcvi-cap-french) Production d’un modèle pour le hareng du Pacifique dans la région d’évaluation des stocks majeur de la côte ouest de l’île de Vancouver.
Voir la figure \@ref(fig:storyboard-hg) pour la description.

```{r storyboard-wcvi, out.height="6.4in", fig.asp=1, fig.cap = ifelse(french, "(ref:storyboard-wcvi-cap-french)", "(ref:storyboard-wcvi-cap)")}
...
(ref:harvest-rate-cap) Time series of effective harvest rate $U$ for Pacific Herring from `r major_start_yr` to `r major_end_yr` in the major stock assessment regions.
Effective harvest rate in year $t$ is calculated as $U_t = C_t/(C_t + \mli{SB}_t)$ where
...
(ref:harvest-rate-cap-french) Série chronologique du taux de récolte effectif pour le hareng du Pacifique de `r major_start_yr` à `r major_end_yr` dans les régions d’évaluation des stocks majeurs.
Le taux de récolte effectif pour l’année $t$ est calculé comme $U_t = C_t/(C_t + \mli{BR}_t)$ où
$C_t$ est est la prise pour l’année $t$, et
$\mli{BR}_t$ est la biomasse reproductrice estimée pour l’année $t$.
...
(ref:proj-biomass-density-cap) Projected spawning biomass assuming no fishing in `r assess_yr+1` $\mli{SB}_{`r assess_yr+1`}$ in thousands of tonnes (t) for Pacific Herring in the major stock assessment regions.
Solid and dashed black lines indicate medians and `r ci_level*100`% confidence intervals for $\mli{SB}_{`r assess_yr+1`}$, respectively.
Vertical red lines and shaded red areas indicate medians and `r ci_level*100`% confidence intervals for the limit reference point $0.3\mli{SB}_0$, respectively,
where $\mli{SB}_0$ is estimated unfished spawning biomass.

(ref:proj-biomass-density-cap-french) Biomasse reproductrice projetée en supposant qu’il n’y aura pas de pêche en `r assess_yr+1` $\mli{BR}_{`r assess_yr+1`}$ en milliers de tonnes pour le hareng du Pacifique dans les régions d’évaluation des stocks majeurs.
Les lignes noires pleines et tiretées indiquent les médianes et les intervalles de confiance de `r ci_level*100`%  pour $\mli{SB}_{`r assess_yr+1`}$, respectivement.
Les lignes et rectangles ombragés rouges indiquent les médianes et les intervalles de confiance de `r ci_level*100`% pour le point de référence limite, $0,3\mli{BR}_0$, où $\mli{BR}_0$ est la biomasse non exploitée estimée.

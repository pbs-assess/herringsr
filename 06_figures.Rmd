...
Panneau (a): ajustement du modèle (estimations de la densité a posteriori maximale, DPM; lignes) aux données mises à l’échelle de relevé du frai (points).
...
Panneau (d): estimation a posteriori de la biomasse reproductrice $\mli{BR}_t$ pour chaque année $t$.
...
Panneau (f): diagramme de phase de la production de la biomasse reproductrice ($\frac{\mli{BR}_{t+1} - \mli{BR}_t + C_{t+1}}{\mli{BR}_t}$) pour la période du relevé de plongée (de `r new_surv_yr` à `r assess_yr-1`; DPM).

...
Panel (a): model fit (median; lines) to scaled spawn survey data (points).
...
Panel (f): phase plot of spawning biomass production for the dive survey period (`r new_surv_yr` to `r assess_yr-1`; maximum posterior density estimates).
...
Panneau (a): ajustement du modèle (médian; lignes) aux données mises à l’échelle de relevé du frai (points).
...
Panneau (f): diagramme de phase de la production de la biomasse reproductrice pour la période du relevé de plongée (de `r new_surv_yr` à `r assess_yr-1`; estimations de la densité a posteriori maximale).
